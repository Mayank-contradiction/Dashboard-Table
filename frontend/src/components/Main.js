    ...
    <>
        ...
        <div className="d-flex justify-content-center align-items-center inner-container text-center text-light">
            <div>
                ...
                <br/>
                {!userLoginLoading && userLoginMessage && userLoginMessage ? <Link className="btn btn-lg btn-outline-light m-2" to={`${userLoginMessage.role === 'user' ? '/userdashboard' : 'admindashboard'}`} role="button">Dashboard</Link> : <Link className="btn btn-lg btn-outline-light m-2" to='/signup-login' role="button">Log In</Link>}
            </div>
        </div>
    </>
    )
